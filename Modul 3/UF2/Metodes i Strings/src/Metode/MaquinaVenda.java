//Revisar Tiquet i Pagament i SeguirComprar
package Metode;
...
public class MaquinaVenda {
    ...
    private static void Tiquet(Scanner sc) { // Mètode per fer tiquet
        do { //Bucle per si l'usuari posa una altre cosa
            ...
            switch (resposta) {
                case "S": //Si l'usuari vol tiquet fa servir el mètode ImprimirTiquet
                    ImprimirTiquet();
                    TipusBitllet(sc);
                    break;
    ...
    private static double SeguirComprant (Scanner sc){ // Mètode per seguir comprant bitllets
        comptador = 1;
        total = preu * comptador;
        numeroBitllets = numeroBitllets + comptador;
        preuTotal = preuTotal + total;
